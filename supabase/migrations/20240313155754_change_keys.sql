...
UPDATE reading_list_items SET ol_book_id = (SELECT legacy_book_mappings.ol_book_id FROM legacy_book_mappings WHERE legacy_book_mappings.legacy_book_id = reading_list_items.book_id LIMIT 1) WHERE reading_list_items.ol_book_id IS NULL;

ALTER TABLE reviews ADD COLUMN IF NOT EXISTS ol_book_id INTEGER REFERENCES ol_books (id);
UPDATE reviews SET ol_book_id = (SELECT legacy_book_mappings.ol_book_id FROM legacy_book_mappings WHERE legacy_book_mappings.legacy_book_id = reviews.book_id LIMIT 1) WHERE reviews.ol_book_id IS NULL;
